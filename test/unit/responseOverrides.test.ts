...
describe('validating a response override', () => {
  ...
  it('takes a status of 100–599, a delay of up to a minute, and header changes a header rule could make', () => {
    const ok = { status: 503, delayMs: 2000, headers: [{ operation: 'set' as const, name: 'X-Mock', value: '1' }] };
...
describe('answering fetch() and XHR', () => {
  ...
  it('serves its body with its status, and its header changes on top of the upstream headers', async () => {
    const o = response({
      ...
      response: { status: 503, delayMs: 0, headers: [{ operation: 'set', name: 'X-Mock', value: 'cart-v2' }, { operation: 'remove', name: 'access-control-allow-origin', value: '' }] },
    });
  ...
  it('holds the answer back for its delay', async () => {
    vi.useFakeTimers();
    try {
      const { transport } = await engineWith([response({ response: { status: 200, delayMs: 1000, headers: [] } })]);
      paused(transport);
...
describe('OverrideStore with response overrides', () => {
  ...
  it('keeps a response as .json with its request match and settings, across instances', async () => {
    ...
    await a.load();
    const o = await a.create({ ...input, request: { method: 'POST', operation: 'GetCart' }, response: { status: 201, delayMs: 500, headers: [] } });
    expect(await readFile(join(dir, 'files', `${o.id}.json`), 'utf8')).toBe('{"a":1}');
    await a.update(o.id, { response: { status: 404, delayMs: 0, headers: [{ operation: 'set', name: 'X-A', value: '1' }] } });

    const b = new OverrideStore(dir);
    await b.load();
    expect(b.meta(o.id)).toMatchObject({ request: { method: 'POST', operation: 'GetCart' }, response: { status: 404, delayMs: 0, headers: [{ operation: 'set', name: 'X-A', value: '1' }] } });
  });
  ...
  it('fills in any method and a plain 200 when they are left out', async () => {
    ...
    expect(o).toMatchObject({ request: { method: '*', operation: '' }, response: { status: 200, delayMs: 0, headers: [] } });
  });

  it('refuses a bad status, and a request match on a file override', async () => {
    const store = new OverrideStore(dir);
    await store.load();
    await expect(store.create({ ...input, response: { status: 700, delayMs: 0, headers: [] } })).rejects.toThrow(/status/);
    ...
    await expect(store.update(script.id, { response: { status: 200, delayMs: 0, headers: [] } })).rejects.toThrow(/response override/);
  });
  ...
  it('falls back to the defaults for settings a hand edit broke', async () => {
    ...
    const b = new OverrideStore(dir);
    await b.load();
    expect(b.get(o.id)).toMatchObject({ request: { method: 'GET', operation: '' }, response: { status: 200, delayMs: 0, headers: [] } });
  });
});
