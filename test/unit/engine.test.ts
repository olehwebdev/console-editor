...
async function setup(overrides: Override[] = [], settings = { ...DEFAULT_SETTINGS }, fallbackFetch?: (url: string) => Promise<string>) {
...
describe('source map headers', () => {
  ...
  it('keeps the upstream SourceMap header of a file served from an override, although the served copy has none', async () => {
    const { transport, engine } = await setup([override({})], { ...DEFAULT_SETTINGS }, async () => 'upstream();');
