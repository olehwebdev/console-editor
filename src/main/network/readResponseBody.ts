...
import { EVENT_SOURCE_TYPE, READ_TIMEOUT_MS, TEXT_MIME } from './constants';
 ...
 * one ends it for the page; an `EventSource` is one before its response comes too), nor one still
 * arriving or that never came; one Chromium no longer holds is `gone`. A text body is decoded with
 * its charset; any other is only said to be binary.
 */
export async function readResponseBody(transport: CdpTransport, entry: TrackedRequest): Promise<NetworkBody> {
  const { row } = entry;
  if (row.type === EVENT_SOURCE_TYPE || row.mimeType.toLowerCase().includes(EVENT_STREAM_MIME_TYPE)) return { available: false, gap: 'stream' };
