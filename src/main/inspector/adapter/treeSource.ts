 ...
 * React's roots (the hook's, else the containers React marks) and Vue's apps.
...
export const TREE_JS = `
  ...
  const treeTop = () => [
    ...reactRoots().flatMap((root) => reactKids(root.current).map((ref) => ({ framework: 'react', ref }))),
  ...
  const treeLocate = (path) => {
    ...
    const found = element && (attempt(reactFind, element) || attempt(vueFind, element));
