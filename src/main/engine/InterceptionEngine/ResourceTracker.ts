...
export class ResourceTracker {
  ...
  /** Network requestId -> the upstream source map header of a file served from an override (shared like `servedBy`). */
  private readonly upstreamSourceMaps: Map<string, string>;
  ...
  constructor(private readonly ctx: ResourceTrackerContext) {
    ...
    this.upstreamSourceMaps = ctx.opts.upstreamSourceMaps ?? new Map();
  ...
  /** The upstream response of this network request, which an override answered, named a source map. */
  markUpstreamSourceMap(networkId: string, value: string): void {
    this.upstreamSourceMaps.set(networkId, value);
  }
  ...
  responded(p: ResponseReceivedParams): void {
    ...
    const sourceMap = this.upstreamSourceMaps.get(p.requestId) ?? sourceMapHeader(headerEntries(p.response.headers));
