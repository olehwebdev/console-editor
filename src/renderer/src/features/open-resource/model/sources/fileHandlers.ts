...
export const FILE_HANDLERS: FileHandlers = {
  ...
  found: async ({ bundleHash, bundle, mapUrl, map }, bundleUrl) => {
    ...
    try {
      ...
      return { status: 'ready', bundleHash, mapUrl, sources: reply.sources, browseOnly: bundle === null, mismatch: false, checked };
    } catch (err) {
