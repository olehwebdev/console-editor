 ...
 * with none just has no original). Null when there is none; undefined when it couldn't be told now (the
 * worker lost the map again), so it is asked again later.
 */
export async function findOriginal(location: CodeLocation): Promise<OriginalPlace | null | undefined> {
  ...
  if (isMiss(reply)) return isMiss(reply, 'unloaded') ? undefined : null;
