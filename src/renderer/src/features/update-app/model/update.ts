...
function announce(update: AvailableUpdate): void {
  ...
  toast({
    ...
    description: update.install === 'auto' ? 'It installs when you restart the app.' : 'Download it, then replace this copy.',
...
function announceReady(update: AvailableUpdate, file: string | undefined): void {
  if (update.install === 'auto') {
    toast({
      ...
      description: 'Restart to finish. Unsaved edits are kept as drafts.',
      tone: 'success',
