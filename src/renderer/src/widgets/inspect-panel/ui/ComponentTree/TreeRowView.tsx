...
import { linkNameAt, useOrigin, useTreeStore, type TreeRow } from '@/entities/inspector';
 ...
 */
export const TreeRowView = memo(function TreeRowView({ row, fresh }: { row: Extract<TreeRow, { kind: 'node' }>; fresh: boolean }) {
  const origin = useOrigin(row.node.location);
  ...
  return (
    ...
    >
      ...
      <button type="button" onClick={() => void openTreeNodeAt(row.path)} className="flex h-full min-w-0 flex-1 items-center gap-2 text-left">
        <span className="min-w-0 truncate">{linkNameAt(row.node, framework, origin)}</span>
