...
- A response override answers after the server has: the request is still sent, so a POST still creates what it creates. An event stream it matches is replaced as a whole, which ends it.
...
- [ ] Network breakpoints, and answering a request without sending it
