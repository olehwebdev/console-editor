...
| U18 | I block a request (an analytics script, a slow third-party iframe) from the file tree, and the page reloads without it | ✅ (§6.3) |
| U19 | I change a response's headers (remove a CSP or X-Frame-Options, set Cache-Control), or let a page call an API cross-origin, without touching the server | ✅ (§6.3) |
