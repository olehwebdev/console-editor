...
test/
  ...
  renderer/          resource tree building, palette fuzzy matching, session, workspaces, rules, original sources
                     (parsing, positions through pretty-printing, tabs, jumps, tree), the Network panel and
                     response tabs
  integration/       engine, rules, iframe and worker sessions against real Chromium + fixture site, source maps
                     in every form, the network log and response overrides
...
- Network panel ([NETWORK_PANEL.md](NETWORK_PANEL.md)): ✅ the page's requests (§6.10) and response overrides for fetch/XHR with a JSON editor (§6.3, §7). Still to come: breakpoints, answering without sending the request, patch mode.
