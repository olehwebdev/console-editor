...
| U26 | I pick an element in the page, in any frame, and see the React or Vue component that rendered it: the file and line it is defined in (a production build's too, through its source map), its props, state and contexts, the element's handlers and the components above it; I open the original or the bundle code there | ✅ (§6.13) |
...
src/
  ...
  main/
    ...
    inspector/       InspectorService/ (the coordinator), StackTracker.ts (the page stack: each frame's libraries, looked
                     ...
                     every session, what is under the pointer), reading/ (a picked element's component, and where its
                     functions are defined), adapter/ (the page-side code that reads React and Vue) (§6.13); and the
                     checks on what the page answers
    network/         NetworkLog (the page's requests on every CDP session, for the Network panel: rows, batches,
                     bodies read on demand, WebSockets and their messages) (§6.10), HeldRequests (what breakpoints hold, until
                     let go) (§6.11)
  ...
  renderer/src/      React UI, Feature-Sliced Design (see DESIGN_SYSTEM.md §6):
    ...
    widgets/         title-bar, activity-bar, explorer, editor-panel, page-preview, status-bar, settings-panel, command-palette,
                     console-panel, actions-panel, network-panel, inspect-panel (the Inspect view)
    features/        open-resource (also original sources and the jumps between them and bundles), save-override,
                     ...
                     inspect/ (a group: stack, the Page stack; pick, picking an element and the Component page)
    ...
    shared/          api (preload bridge), ui (design system), monaco (+ the JSON service and per-model schemas),
                     lib (format and source-map workers, overlays, motion, JSON schemas inferred from a response),
                     config
test/
  ...
  helpers/           Chromium launcher with the app's flags, WebSocket CDP harness, bundleApp.ts (a fixture app
                     bundled with Vite: minified with a source map, or a development build)
...
| Reading a component | `Runtime.callFunctionOn` of `ADAPTER_SOURCE` on the element, in the page's main world, `silent`. React: the fiber on the element or its nearest ancestor (`__reactFiber$…`), up through its `return`s to the function, class, `memo` and `forwardRef` components (the chain, at most 40); props from `memoizedProps`; hooks from the `memoizedState` list (state, a store, a ref, a memo; effects left out), a class's `state`; each context it reads (`dependencies.firstContext`) with its nearest provider's `value` and the component that renders it. Vue 3: `__vueParentComponent` in development builds; production builds don't set it, so the app's vnode tree is walked from the element it is mounted on (`_vnode`, at most 20,000 vnodes) to find the element's component; its props, `setupState` and `data`, what it provides, and the chain through `parent`. The element's handlers: the `on…` props React keeps on it (`__reactProps$…`), or its vnode's. The adapter answers with previews and the functions it named; `Runtime.getProperties` gives each function's `[[FunctionLocation]]` (script id, line and column) |
| Script URLs | Only `Debugger.scriptParsed` names a script id. When one isn't known yet, `Debugger.enable` replays it for every script already parsed, with `Debugger.setSkipAllPauses` so a `debugger` statement can't stop the page, and `Debugger.disable` right after |
...
| The original | In the renderer, each function's place in the bundle as served goes through the bundle's source map (the source-map worker's `toOriginalRaw`; a map not loaded yet is loaded quietly): the original file and line, and the function's name, from the map's `names` or, when it has none (Vite's minifier writes none), read off the original's text where the definition starts (`function Name`, `class Name`, `const Name = (…) =>`, a method or a property). A React component is then named after its original; Vue components carry their own names |

**UI.** The **Inspect** view in the rail shows what is under the pointer while picking, then the component picked and its chain, and the page stack in short. The **Component** page tab (one per workspace) is titled with the component's name and shows the element (hovering it highlights it in the page), its frame, the source card (**Open original**, read-only at the component's line; **Go to bundle code**, the pretty-printed bundle at its definition), props, state, context and who provides it, the element's handlers with where each is defined, and the chain, where clicking a component reads that one. Picking again replaces it.
...
| Layout | Sidebar and preview are fitted to the window (the editor keeps at least 240 px; panel minimums give way below that, e.g. when zoomed in). Hiding the preview takes the native page view out of the window with it. The bottom pane shows the console or the network panel, one tab each, and Ctrl/Cmd+J brings back the one shown last. Visibility, sizes and the bottom pane's tab are saved on every change |
...
- The component inspector's adapter runs in the main world of the frame picked in, where the page could change what it answers: the answer is checked the same way (§6.13), and names, previews and keys are only ever shown as plain labels. Reading where a function is defined turns the page's Debugger domain on for a moment, with pauses skipped, and off again.
...
- Component inspector ([research](INSPECTOR_RESEARCH.md)): ✅ the page stack, which UI library, framework, state library and bundler each frame runs (§6.12); ✅ the React or Vue component behind an element, its original file (production builds too, through source maps), props, state, context, handlers and chain (§6.13). Still to come: a tree of the page's components, Vue 2 and Angular components, and how data reaches a component and why it re-renders.
