...
1. Renderer: wait for running saves, stop the session sync, write what is pending (as on close; if a write failed, ask before going on), then close the file tabs without deleting their drafts. Pages (What's New) stay open.
2. Main (`WorkspaceController.switchTo`): load `about:blank` and clear the history, so nothing the old page does from then on (an in-page navigation, a title, a favicon) is taken for the next workspace's; make the workspace active; point the engine at its overrides (`Fetch` patterns recomputed, `overrides-changed` sent); send `workspaces-changed`; load its last page, clearing the history again once it has loaded, so Back never leads into another workspace's pages.
3. Renderer: reload the workspaces and overrides, reopen the tabs of whichever workspace is now active (the old one again, if switching failed) the way a start does, and sync again.
