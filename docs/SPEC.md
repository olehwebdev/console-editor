...
| U24 | I pick an element in the page, in any frame, and see the React or Vue component that rendered it: the file and line it is defined in (a production build's too, through its source map), its props, state and contexts, the element's handlers and the components above it; I open the original or the bundle code there | ✅ (§6.11) |
| U25 | I browse a frame's components as a tree, open any of them, and set a value of a component's state to see the page with it | ✅ (§6.11) |
| U26 | I record renders and see, commit by commit, what triggered it and why each React component rendered (its props, its state, a store, a context, or its parent), or was skipped | ✅ (§6.12) |
...
src/
  ...
  main/
    ...
    inspector/       InspectorService/ (the coordinator), StackTracker.ts (the page stack: each frame's libraries, looked
                     at once it loads), the page-side detector and React hook stand-in (§6.10); picking/ (inspect mode in
                     every session, what is under the pointer), reading/ (a picked element's component, where its
                     functions are defined, the Components tree, setting state), adapter/ (the page-side code that reads
                     React and Vue, walks their trees and sets state) (§6.11); renders/ (recording React's commits) and the
                     stand-in's summarizer (§6.12); and the checks on what the page answers
  ...
  renderer/src/      React UI, Feature-Sliced Design (see DESIGN_SYSTEM.md §6):
    ...
    widgets/         title-bar, activity-bar, explorer, editor-panel, page-preview, status-bar, settings-panel, command-palette,
                     console-panel, actions-panel, inspect-panel (the Inspect view, with the Components tree), renders-panel
    features/        open-resource (also original sources and the jumps between them and bundles), save-override,
                     ...
                     pick, picking an element, the Component page and setting state; tree, the Components tree; renders)
    entities/        page, settings, override, editor-tab (+ Monaco model registry, page tabs and their drafts,
                     ...
                     hook names, the Components tree, the Renders log)
    shared/          api (preload bridge), ui (design system), monaco, lib (format and source-map workers,
                     overlays, motion; the source-map worker also reads hook names off originals), config
test/
...
| Components tree | A frame's components a level at a time, read in its main world (where the console's contexts say, so it needs **Record the console**) by the adapter's `tree`: React's roots from the hook stand-in (`getFiberRoots`, as the extension's hook has), else from the containers React marks (`__reactContainer$…`, in the first 5,000 elements), and Vue apps (`[data-v-app]`; a production build sets no `app._instance`, so the container's root vnode leads to it). A node's children are the nearest components under it; a node is addressed by its path of indexes from the top, at most 500 listed per level. A node opens as a pick of its first element, at its component's depth in that element's chain (`locate`), and highlights as that element; a pick tells its path, so the tree reveals it |
...
| Menu | App menu replaces Electron's default, so Ctrl/Cmd+R reloads **the site**, not the editor. Undo/redo/select-all are routed to Monaco (or to the website, or the website window's address bar, when one of those has focus). Page DevTools: Ctrl/Cmd+Shift+J; editor DevTools: Ctrl/Cmd+Alt+I. Ctrl/Cmd+B toggles the sidebar. **View › Website in Its Own Window** moves the website either way (§7.1). **View › Go to Original Source or Bundle Code** (Ctrl/Cmd+Shift+M) jumps whichever way applies, and says so on other tabs. **View › Pick an Element** (Ctrl/Cmd+Shift+C) starts or stops picking (§6.11) |
...
- The component inspector's adapter runs in the main world of the frame picked in, where the page could change what it answers: the answer is checked the same way (§6.11), and names, previews and keys are only ever shown as plain labels. Reading where a function is defined turns the page's Debugger domain on for a moment, with pauses skipped, and off again. Setting state runs in the page with a value the renderer wrote as JSON, parsed by main and passed as an argument, never as code.
- While renders are recorded, each document has the binding's function as a global, which the page can see and call: what it hands over is checked like any other answer (§6.12), and it is taken out of loaded documents when recording stops.
...
- Component inspector ([research](INSPECTOR_RESEARCH.md)): ✅ the page stack, which UI library, framework, state library and bundler each frame runs (§6.10); ✅ the React or Vue component behind an element, its original file (production builds too, through source maps), props, state, context, handlers and chain (§6.11); ✅ the Components tree, setting state, hook names from originals and source-map coverage (§6.10, §6.11); ✅ Renders, why each React component rendered (§6.12). Still to come: Vue's renders, Vue 2 and Angular components, a data-flow view (what comes into a component and what it sets or calls), and **Load a source map…** for a bundle whose map isn't published.
