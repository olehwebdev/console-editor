...
src/
  ...
  main/
    ...
    inspector/       InspectorService/ (the coordinator), StackTracker.ts (the page stack: each frame's libraries, looked
                     ...
                     functions are defined), adapter/ (the page-side code that reads React and Vue) (§6.11); and the
  ...
  renderer/src/      React UI, Feature-Sliced Design (see DESIGN_SYSTEM.md §6):
    ...
    widgets/         title-bar, activity-bar, explorer, editor-panel, page-preview, status-bar, settings-panel, command-palette,
                     console-panel, actions-panel, inspect-panel (the Inspect view)
    features/        open-resource (also original sources and the jumps between them and bundles), save-override,
                     ...
                     pick, picking an element and the Component page)
...
- The component inspector's adapter runs in the main world of the frame picked in, where the page could change what it answers: the answer is checked the same way (§6.11), and names, previews and keys are only ever shown as plain labels. Reading where a function is defined turns the page's Debugger domain on for a moment, with pauses skipped, and off again.
...
- Component inspector ([research](INSPECTOR_RESEARCH.md)): ✅ the page stack, which UI library, framework, state library and bundler each frame runs (§6.10); ✅ the React or Vue component behind an element, its original file (production builds too, through source maps), props, state, context, handlers and chain (§6.11). Still to come: a tree of the page's components, Vue 2 and Angular components, and how data reaches a component and why it re-renders.
