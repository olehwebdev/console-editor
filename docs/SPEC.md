...
src/
  ...
  main/
    ...
    WorkspaceController/  workspaces: switching, the page URL, title and favicon each remembers (§5.1)
    ...
    engine/          PageInterception/ (one engine per CDP session: page, iframes, workers; hands each frame
                     ...
                     header edits), transform/ (SRI/source maps/headers),
    ...
    store/           OverrideStore/, SessionStore/, RuleStore/ (each a store with its file and record helpers), SettingsStore.ts,
  ...
  renderer/src/      React UI, Feature-Sliced Design (see DESIGN_SYSTEM.md §6):
    ...
    features/        open-resource, save-override, format-document, compare-changes, toggle/delete-override,
                     ...
                     toggle; delete)
    entities/        page, settings, override, editor-tab (+ Monaco model registry, page tabs and their drafts), resource,
...
test/
  ...
  integration/       engine, rules, iframe and worker sessions against real Chromium + fixture site
