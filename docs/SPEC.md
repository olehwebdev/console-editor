...
| U22 | I stop a fetch() or XHR before it goes out or before the page gets its response, change it, and send it on, send it as it was or fail it; and I answer an API call without it ever reaching the server | ✅ (§6.3, §6.11) |
...
src/
  ...
  main/
    ...
    PageWindow/      which window shows the site's view: the editor's, or a window of its own (§7.1), and where
                     that window opens (placePageWindow.ts)
    ...
    network/         NetworkLog (the page's requests on every CDP session, for the Network panel: rows, batches,
                     bodies read on demand) (§6.10), HeldRequests (what breakpoints hold, until let go) (§6.11)
    ...
    store/           OverrideStore/, SessionStore/, RuleStore/, ActionStore/ (each a store with its file and record helpers), SettingsStore.ts,
                     PageWindowStore/ (the website window's place), WriteQueue.ts, writeAtomic.ts and shared sanitizers
  ...
  renderer/src/      React UI, Feature-Sliced Design (see DESIGN_SYSTEM.md §6):
    app/             entry, providers, event bridge (main → stores), styles/tokens, component gallery; the view each
                     window shows (views/: editor, gallery, website window) and the website window's bridge
    ...
    features/        open-resource (also original sources and the jumps between them and bundles), save-override,
                     ...
                     toggle; delete), action/ (a group: run, edit), edit-response-rule, network/ (a group:
                     filter, clear)
...
2. **Override found and the upstream response is not a redirect** (3xx + `Location`):
   ...
   - a **response override** (kind `Fetch`) has no `originalHash` (an API's responses change on every call), so upstream is never read; it answers with its own status and its header changes applied to the headers above (before rules), after its delay. One that matches an event stream replaces it as a whole, which ends it (the page's `EventSource` reconnects and is answered again); a stream it pauses but doesn't answer is continued unread;
...
- Network panel ([NETWORK_PANEL.md](NETWORK_PANEL.md)): ✅ the page's requests (§6.10), response overrides for fetch/XHR with a JSON editor (§6.3, §7), answering without sending the request (§6.3), breakpoints and Copy as fetch (§6.11). Still to come: patch mode, quick edits for UI states, throttling, HAR.
...
- ✅ Console panel for the page and every frame in it (§6.7), and actions: code kept to run in a frame with one click (§6.9). Still to come ([research](ACTIONS_RESEARCH.md)): the Actions view in a window of its own, parameters in actions, sending a message to a frame without writing code, a message log of `postMessage` between frames, waiting for a log, scenarios, showing a frame in the page, reloading or retargeting one frame, network and storage per frame.
