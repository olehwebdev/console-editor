...
- Packaged builds drop `--remote-debugging-port`/`--remote-debugging-pipe` unless `CONSOLE_EDITOR_USER_DATA` points elsewhere, as Chrome does for its default profile: otherwise any local program could start the app with a port and read the site view's logins.
