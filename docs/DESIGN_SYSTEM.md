...
| `Input` | `leading?`, `trailing?`, `size?`, `mono?`, `invalid?` + native props (`ref` is the `<input>`) | focus ring = accent at 40%; `invalid` (or `aria-invalid`) turns frame and ring `--danger` |
| `FieldError` | `id`, `message?` + `<p>` props | what is wrong with a field, under it (`text-danger`, 12 px, `role="alert"`); renders nothing without a message; the field names it in `aria-describedby` |
...
| `UrlMatcherFields` | `value: UrlMatcher`, `onChange`, `onEnter?`, `error?`, `patternRef?`, `testIdPrefix?`, `autoFocus?` | the match type menu, the pattern and "ignore ?query", as siblings the caller lays out in a wrapping row (the pattern's `error` on a line of its own); shared by an override's match row and rule pages |
...
| `PaneTabs` | `tabs: {id, label, count?, live?}[]`, `value`, `onChange`, `label`, `caps?` | a pane's views as a strip of words (Console / Network / Renders, a request's Headers / Payload / Response): the shown one in `--fg` over a 2 px ember underline, the others `--fg-subtle`; one tab stop, arrows move and show (WAI-ARIA tabs, automatic activation); `caps` sets them as the pane's heading (`label-caps`). A tab's `count` (what waits in it: requests paused at a breakpoint) shows beside its label in the warning tone while above zero, and `live` (something running in it: the Renders log recording) a small red dot. A toolbar that holds them lets its filter give way first (a high `shrink`); then the strip scrolls sideways |
...
- **Page tabs keep their own edits.** A tab that isn't a file (What's New, a rule page, a new-rule page) is a `PageTab`, a union by `page` kind. Each kind declares whether it belongs to the app or to the workspace (`PAGE_SCOPES`: workspace pages close on a switch), whether it holds edits (`PAGE_DIRTY_CHECKS`), and which view renders it (`PAGE_VIEWS` in `widgets/editor-panel`), so a new kind fails typecheck until it has all three. A rule page's unapplied edits live in its form, a react-hook-form control made outside React (`createFormControl`) and kept by page id in `features/rule/edit/model/ruleForms`, so they survive switching tabs and Save reaches them; the tab only holds `dirty`, which the form's `subscribe` keeps in step. The form's defaults are the saved rule: the rule's own Apply landing rebases what was typed meanwhile onto the new version (`reset(saved, { keepValues: true })`, so header rows keep their keys and focus).
- **Non-serializable objects stay out of stores**: Monaco models and editor instances live in registries (`entities/editor-tab/model/models/`, `shared/monaco/editors/`) keyed by tab id, rule pages' forms in theirs; the store only holds metadata (dirty, saved version, diff mode). Decoded source maps live in the source-map worker; `entities/source-map` keeps each bundle's state and file list.
