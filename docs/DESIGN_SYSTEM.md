...
| `PanelResizer` | `onResize(delta, total)`, `onResizeStart?`, `onResizeEnd?`, `onReset?`, `orientation?`, `value?/min?/max?`, `hairline?` | window-splitter handle between panels: an 8 px hit area with a grip of three 2 px dots (`--fg-muted`, brightening to `--fg`) that says it can be dragged, and a 2 px accent line on hover (after 200 ms), focus and drag. Grip and line stay within the 2 px holding the panel's border, since what lies past it may be the native page view or a clipped edge |
