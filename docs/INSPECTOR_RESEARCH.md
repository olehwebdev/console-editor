...
- **Getting commits out:** batched in the page every 100 ms and pushed through `Runtime.addBinding`, capped at the last 2,000 commits in the app. A binding is a global the page can see, which is another reason for the setting. Probed: `Runtime.addBinding` takes only while the session's Runtime domain is on, so Renders needs **Record the console**.
...
3. **Renders:** the Components tree, source-map coverage in the stack, commit summaries from the stand-in, the Renders tab, why it rendered, setting state, hook names from originals. *Built: SPEC §6.12–§6.14. Differences from the plan: the binding's being in a document is what turns the stand-in's recording on (no flag to keep in step, and a document loaded while recording records its first mount); a removed binding leaves its function in documents already loaded, so stopping deletes it there. Only React's renders are recorded: Vue's production builds leave no trail, and wrapping each instance's update is still unprobed. The Component page shows a component's renders as a section rather than a tab, and its Data flow tab is left for later. Coverage comes from `Debugger.scriptParsed`'s `sourceMapURL`, which carries the `SourceMap` header's too. Reading a component now finds the fiber copy on screen (React DevTools' `findCurrentFiberUsingSlowPath`): an element's key keeps the copy it was created with, so after an update the other copy is the current one.*
4. **Angular and the rest:** `ng` in development, the registry in production (labelled), custom elements, plain DOM listeners, **Load a source map…**. *Built: SPEC §6.8, §6.13, with Vue 2 too. Probed on Angular 22.2: `__ngContext__` is on each view's first elements as well as on hosts, so a host is told by its declaring view holding a component view (`TView.type` 1) that starts with it, not by the property; views keep their id in slot 19; the component instance carries `__ngContext__` too; a production build's `window.ng` holds only the compiler facade; `Runtime.queryObjects` works without the Runtime domain on (about 100 ms on a small app). `DOMDebugger.getEventListeners` gives handlers only for a handle in a named object group. Vue 2 puts the outermost component on a shared root element. V8 places a class that has its own constructor at that constructor. Left for later: Angular's listeners by name (its wrapper's closure holds them) and its injected services.*
5. **Data flow:** Redux, Pinia and NgRx timelines; requests by component (`Network.requestWillBeSent`'s initiator stack through the maps); "Save as action" for setting state; a profiler view. *Built: SPEC §6.10, §6.13–§6.15, with Redux Toolkit, Zustand's devtools and Vuex too. Differences from the plan: the store actions are a tab of their own (**Stores**) beside Renders, recorded apart from it; a commit names the store action just before it (within 250 ms) rather than one in the same task. Probed: Redux Toolkit's `configureStore` composes the extension in even in production (`devTools` defaults on); NgRx's StoreDevtools sends the app's action wrapped in `PERFORM_ACTION` through an RxJS pipeline that puts the app's call about 40 deep in the stack, and calls a `send` on the extension object itself; Zustand's devtools is off in production builds unless `enabled`. A request's initiator stack comes with the `Network` domain alone, but only its synchronous part: async parents need `Runtime.setAsyncCallStackDepth` (with Runtime on), which costs every page, so they are left out. The profiler's times are each component's `selfBaseDuration`, which React measures in development and profiling builds only. Save as action finds the element by selectors (one per shadow root) and the component the way the adapter does, so it works after a reload, but not in a closed shadow root or Angular's production build.*
