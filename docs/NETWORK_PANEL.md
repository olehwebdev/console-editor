...
**Status.** All three phases (§8) are built: the Network tab, response overrides and the JSON editor; answering without sending the request, breakpoints and Copy as fetch; patch mode, quick edits, network speed, HAR export and import, and WebSocket messages; as SPEC §6.3, §6.10, §6.11 and §7 describe them. §11 to §13 list where the build differs from the plan below. A response's JSON also shows as a tree with in-place edits (§14).
...
- **Tree view.** A response's JSON as a tree with in-place edits, and quick edits for UI states (empty every array, lengthen every string, null a field, fail with 500, add a delay). Both are built (§13, §14).
...
## 14. The response tree as built

- **Instead of the text, not beside it.** The header's tree button swaps the text for the tree, per tab (the text editor stays mounted under it, with its undo and view state). Beside the text, an editor column a third of the window wide would hold neither.
- **The text stays the source.** The tree is read from the tab's text by the same lossless parser as patch mode, again after every edit (a cached snapshot per model version), and every change in the tree is one undoable edit of that text: only the value, key or member touched changes, so undo, the dirty mark, drafts, saving, the diff and patch mode all work as for typed edits. A removed member takes its comma with it; an added one is a `null` under a free key, written as the last one is (on its own line with its indent when that one is, compact when it is).
- **A value is typed as JSON**, so it can change kind (a string to a number, a list to `[]`); a string's inside is selected at first, so typing replaces its text. Keys are typed as plain text.
- **Keyboard** as WAI-ARIA's tree: arrows move, open and close, Home and End jump, Enter edits a value or toggles an object or array, F2 renames, Delete removes. The selection follows a renamed key, and moves to the next row after a removal. The tree takes focus as it appears, and the hidden text editor doesn't take it back on a tab switch, so keys never edit text you can't see.
- **Found by driving it:** the field wrote its edit twice (Enter, then the blur as focus went back to the tree), with offsets from before the first, which broke the text on a rename; it now writes once.
