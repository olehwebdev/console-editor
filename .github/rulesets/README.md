...
- **No signed commits.** The maintainer's own commits aren't signed (Claude sessions' are), and a merge commit brings a branch's commits into `main` as they are, so requiring signatures would block the maintainer's pull requests and release branches.
